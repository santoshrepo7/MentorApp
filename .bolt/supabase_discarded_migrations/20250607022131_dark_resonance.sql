/*
  # Complete Mentor Media Implementation

  1. New Tables
    - `mentor_media`
      - `id` (uuid, primary key)
      - `mentor_id` (uuid, references professionals)
      - `media_url` (text)
      - `caption` (text)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on mentor_media table
    - Add policies for mentors to manage their media
    - Allow public viewing of media

  3. Data Population
    - Add default professional images for each mentor
    - Use variety of high-quality Pexels images
    - Add meaningful captions

  4. Implementation Details
    - Handles existing table gracefully
    - Prevents duplicate image entries
    - Uses professional stock photos
    - Maintains data consistency
*/

-- Step 1: Create mentor_media table if it doesn't exist
CREATE TABLE IF NOT EXISTS mentor_media (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  mentor_id uuid REFERENCES professionals(id) ON DELETE CASCADE,
  media_url text NOT NULL,
  caption text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);

-- Step 2: Enable RLS
ALTER TABLE mentor_media ENABLE ROW LEVEL SECURITY;

-- Step 3: Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Mentors can manage their own media" ON mentor_media;
DROP POLICY IF EXISTS "Everyone can view mentor media" ON mentor_media;

-- Step 4: Create RLS policies
CREATE POLICY "Mentors can manage their own media"
  ON mentor_media
  FOR ALL
  TO authenticated
  USING (mentor_id = auth.uid())
  WITH CHECK (mentor_id = auth.uid());

CREATE POLICY "Everyone can view mentor media"
  ON mentor_media
  FOR SELECT
  TO public
  USING (true);

-- Step 5: Drop existing trigger if it exists
DROP TRIGGER IF EXISTS update_mentor_media_updated_at ON mentor_media;

-- Step 6: Add trigger for updated_at
CREATE TRIGGER update_mentor_media_updated_at
  BEFORE UPDATE ON mentor_media
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- Step 7: Function to add default images for mentors
CREATE OR REPLACE FUNCTION add_default_mentor_images()
RETURNS void AS $$
DECLARE
    mentor_record RECORD;
    image_urls text[] := ARRAY[
        'https://images.pexels.com/photos/3184465/pexels-photo-3184465.jpeg',
        'https://images.pexels.com/photos/3184603/pexels-photo-3184603.jpeg',
        'https://images.pexels.com/photos/3184287/pexels-photo-3184287.jpeg',
        'https://images.pexels.com/photos/7516363/pexels-photo-7516363.jpeg',
        'https://images.pexels.com/photos/7516339/pexels-photo-7516339.jpeg',
        'https://images.pexels.com/photos/3184360/pexels-photo-3184360.jpeg',
        'https://images.pexels.com/photos/3184291/pexels-photo-3184291.jpeg',
        'https://images.pexels.com/photos/3184292/pexels-photo-3184292.jpeg',
        'https://images.pexels.com/photos/3184418/pexels-photo-3184418.jpeg',
        'https://images.pexels.com/photos/3184419/pexels-photo-3184419.jpeg'
    ];
    captions text[] := ARRAY[
        'Professional Development Session',
        'Team Leadership Workshop',
        'One-on-One Mentoring',
        'Strategic Planning Meeting',
        'Skills Development Training',
        'Career Guidance Session',
        'Industry Expertise Sharing',
        'Goal Setting Workshop',
        'Performance Review Session',
        'Knowledge Transfer Meeting'
    ];
    selected_indices integer[];
    i integer;
    random_index integer;
BEGIN
    -- Loop through all mentors
    FOR mentor_record IN SELECT id FROM professionals
    LOOP
        -- Only add default images if mentor doesn't have any
        IF NOT EXISTS (
            SELECT 1 FROM mentor_media 
            WHERE mentor_id = mentor_record.id
        ) THEN
            -- Generate 3 unique random indices for this mentor
            selected_indices := ARRAY[]::integer[];
            
            FOR i IN 1..3 LOOP
                -- Generate a pseudo-random index based on mentor ID and loop counter
                random_index := (abs(hashtext(mentor_record.id::text || i::text)) % array_length(image_urls, 1)) + 1;
                
                -- Ensure we don't duplicate indices (simple approach)
                WHILE random_index = ANY(selected_indices) LOOP
                    random_index := (random_index % array_length(image_urls, 1)) + 1;
                END LOOP;
                
                selected_indices := selected_indices || random_index;
                
                -- Insert the image with corresponding caption
                INSERT INTO mentor_media (mentor_id, media_url, caption)
                VALUES (
                    mentor_record.id,
                    image_urls[random_index],
                    captions[random_index]
                );
            END LOOP;
        END IF;
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- Step 8: Execute the function to add default images
SELECT add_default_mentor_images();

-- Step 9: Clean up by dropping the temporary function
DROP FUNCTION add_default_mentor_images();

-- Step 10: Add indexes for better performance
CREATE INDEX IF NOT EXISTS idx_mentor_media_mentor_id ON mentor_media(mentor_id);
CREATE INDEX IF NOT EXISTS idx_mentor_media_created_at ON mentor_media(created_at DESC);