/*
  ...
  1. New Tables
    - stale_appointments: Archives old appointments
      - Mirrors appointments table structure
      - Adds archived_at timestamp
      - Includes all constraints from original table
      - Maintains foreign key relationships

  2. Security
    - Enables RLS on stale_appointments
    - Adds policy for users to view their own stale appointments

  3. Functions
    - move_to_stale_appointments(): Moves old appointments to archive
    - cleanup_old_appointments(): Wrapper function for manual cleanup
*/
...
-- Create a function to run the cleanup
